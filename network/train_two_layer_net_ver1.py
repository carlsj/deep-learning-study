...
for ind_iters in range(0, iters_num):
    #batch_mask = np.random.randint(0, input_size, batch_size)
    #  - train result is not good as using np.random.choice : why?
    batch_mask = np.random.choice(train_size, batch_size)
    x_batch = x_train[batch_mask]
    t_batch = t_train[batch_mask]

    # Compute gradient
    #grad = mnist_nn.numerical_gradient(x_batch, t_batch)
    grad = mnist_nn.gradient(x_batch, t_batch)

    # Update parameters
    for key in ('W1', 'b1', 'W2', 'b2'):
        mnist_nn.param[key] -= lr * grad[key]
